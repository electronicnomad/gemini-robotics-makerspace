...
# Load .env using python-dotenv (same mechanism as the production robot_puppy_core.py),
# with a manual fallback if python-dotenv is not installed.
try:
    from dotenv import load_dotenv
    if os.path.exists(".env"):
        load_dotenv()  # Existing real environment variables take precedence (override=False by default)
        print("✅ Loaded .env via python-dotenv (matching production code).")
    else:
        print("ℹ️ No .env file found in workspace.")
except ImportError:
    print("⚠️ python-dotenv not installed. Falling back to manual .env parsing.")
    if os.path.exists(".env"):
        try:
...
# Read the key from the environment after loading (covers both real env vars and .env)
api_key = os.environ.get("GEMINI_API_KEY")
if api_key:
    print("✅ Found GEMINI_API_KEY in environment/.env.")
